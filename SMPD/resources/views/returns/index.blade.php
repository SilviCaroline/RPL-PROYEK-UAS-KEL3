...
<div class="flex min-h-screen">
    <aside class="w-72 bg-blue-950 text-white p-6 hidden md:block">
        ...
        <nav class="space-y-3">
            ...
            <a href="{{ route('returns.index') }}" class="block px-4 py-3 bg-blue-900 rounded-lg">Pengembalian</a>
                ...
                <tbody>
                    @foreach($returns as $return)
                        <tr class="border-b">
                            ...
                            <td class="p-4">{{ $return['loan_code'] }}</td>
                            <td class="p-4">{{ $return['member_name'] }}</td>
                            <td class="p-4">{{ $return['book_title'] }}</td>
                            <td class="p-4">{{ $return['due_date'] }}</td>
                            <td class="p-4">{{ $return['return_date'] }}</td>
                            <td class="p-4">{{ $return['late_days'] }} hari</td>
                            <td class="p-4">Rp{{ number_format($return['fine'], 0, ',', '.') }}</td>
                            <td class="p-4">
                                @if($return['status'] == 'Tepat Waktu')
                                    <span class="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs">Tepat Waktu</span>
                                @else
