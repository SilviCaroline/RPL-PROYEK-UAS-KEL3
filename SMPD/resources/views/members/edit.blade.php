    ...
    <div class="bg-white rounded-2xl shadow p-8 mt-6">
        ...
        <form action="{{ route('members.update', $member['id']) }}" method="POST" class="grid md:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <input type="text" name="member_code" value="{{ $member['member_code'] }}" class="w-full border rounded-lg px-4 py-3">
            </div>

            <div>
                <label class="block mb-2 font-medium">Nama Lengkap</label>
                <input type="text" name="name" value="{{ $member['name'] }}" class="w-full border rounded-lg px-4 py-3">
            </div>

            <div>
                <label class="block mb-2 font-medium">Email</label>
                <input type="email" name="email" value="{{ $member['email'] }}" class="w-full border rounded-lg px-4 py-3">
            </div>
            ...
            <div>
                ...
                <select name="status" class="w-full border rounded-lg px-4 py-3">
                    <option {{ $member['status'] == 'Aktif' ? 'selected' : '' }}>Aktif</option>
                    <option {{ $member['status'] == 'Nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                </select>
            </div>
